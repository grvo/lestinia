...
fn main() {
    ...
    loop {
        ...
        for event in events {
            match event {
                Event::ClientConnected { ecs_entity } => info!("cliente {} conectado!", uid),
                Event::ClientDisconnected { ecs_entity } => info!("cliente {} desconectado!", uid),

                Event::Chat { ecs_entity, msg } => info!("[cliente {}] {}", uid, msg)
            }
        }
